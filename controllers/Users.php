    ...
    class Users {
        ...
        public function register(){
            ...
            empty($data['usersPwd']) || empty($data['pwdRepeat'])){
                flash("register", "Täida kõik väljad");
                redirect("../signup.php");
            }

            if(!preg_match("/^[a-zA-Z0-9]*$/", $data['usersUid'])){
                flash("register", "Vigane kasutajanimi");
                redirect("../signup.php");
            }

            if(!filter_var($data['usersEmail'], FILTER_VALIDATE_EMAIL)){
                flash("register", "Vigane meil");
                redirect("../signup.php");
            }

            if(strlen($data['usersPwd']) < 6){
                flash("register", "Vigane parool");
                redirect("../signup.php");
            } else if($data['usersPwd'] !== $data['pwdRepeat']){
                flash("register", "Paroolid ei ühti");
                redirect("../signup.php");
            }

            //User with the same email or password already exists
            if($this->userModel->findUserByEmailOrUsername($data['usersEmail'], $data['usersName'])){
                flash("register", "Kasutajanimi või meil on juba kasutusel");
                redirect("../signup.php");
            }
